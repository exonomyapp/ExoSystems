...
*   `POST /api/v1/identity/device/link`
...
*   `POST /api/v1/network/proximity/scan`
    *   **Description**: Activates BLE/mDNS scanning to discover nearby Conscia nodes.
*   `POST /api/v1/network/proximity/handshake`
...
*   `GET /api/v1/messages/direct/{peer_id}`
    *   **Description**: Retrieves the message history for a specific 1:1 conversation.
*   `POST /api/v1/messages/channel/{channel_id}`
...
*   `POST /api/v1/system/services/pgedge/query`
