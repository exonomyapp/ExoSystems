...
The foundation of this architecture relies on several strict technical choices designed to maximize both user autonomy and network resilience:
...
* **Identity and Authentication:** We rely on **Meadowcap** for capability delegation, supplemented by **ExoAuth (The Universal Passport)** for identity synthesis and OAuth recovery anchors. The `did:peer` remains the autonomous root of trust.
* **Availability:** To solve the offline-peer problem, we introduce **Conscia (The Lifeline)**. Always-on nodes act as high-speed mirrors and buffers. These are managed via the **Conscia Management Console (CMC)**.
...
* **Decentralized Identity:** Create a unique user identity without a central authority, ensuring privacy and self-autonomy via `did:peer`.
...
* **Decentralized Groups:** Participate in secure, P2P group chats with cryptographically enforced Meadowcap capabilities.
...
* **Lifeline:** Optional, but encouraged beacon services (**Conscia**) to facilitate offline message delivery and peer discovery without compromising data autonomy. Managed via the dedicated **CMC** dashboard.
