...
class ExoZoom extends StatefulWidget {
  final Widget child;
  const ExoZoom({super.key, required this.child});

  /// Access the zoom state from descendants if needed.
  static double scaleOf(BuildContext context) {
    final state = context.findAncestorStateOfType<_ExoZoomState>();
    return state?._scale ?? 1.0;
  }

  @override
  State<ExoZoom> createState() => _ExoZoomState();
}

class _ExoZoomState extends State<ExoZoom> {
  double _scale = 1.0;
  static const String _zoomKey = 'exo_zoom_factor';
