...
pub async fn get_device_manifest() -> DeviceManifest {
    ...
    // Sync the associated Conscia ID with the network engine (Directly to internal to avoid recursion)
    if let Some(node_id) = &manifest.associated_conscia_id {
        let _ = exotalk_core::network_internal::set_associated_conscia(node_id.clone()).await;
    }
...
pub async fn sign_out_profile() {
    ...
    // Shutdown the active P2P engine
    crate::api::network::shutdown_network().await;
}
        ...
        Ok(data) => {
            match serde_json::from_str::<IdentityVault>(&data) {
                Ok(vault) => {
                    ...
                    println!("Starting Iroh node for DID: {}", vault.did);
                    if let Err(e) = crate::api::network::start_iroh_node(vault.did, vault.secret).await {
                        println!("Failed to start Iroh node: {}", e);
                        sign_out_profile().await;
                        return Err(e);
                    }
                    let _ = init_willow_database().await;
